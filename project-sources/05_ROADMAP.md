...
- fix Rust lexical/fixture defects detectable before native compilation;
- canonical domain-separated full-state digest input, including nonempty structured-key maps;
- complete environment checkpoints with status and limit counters;
- sequential/compositional semantic-event validation;
- exact conformance assertions for visible decision and submitted response;
- typed knowledge acquisition and per-object invalidation provenance;
- native-executor discovery from reachable definition closure;
- verification evidence outside the source archive and archive gate last;
- all V0.2.1 freeze gates green under pinned Rust/Python toolchains.
...
- preserve V0.2.1 semantic contracts;
...
- commit `Cargo.lock` and pass every native/Python freeze gate.

**Exit:** all V0.2.2 gates PASS; M1 unblocked.

## M1 — Closed Deterministic Kernel Shell

- construct/reset synthetic complete `EngineState`;
- one accepted and one rejected synthetic decision path;
...
- representative hierarchical decisions and continuations;
- perspective-bound observations, information state, observed events, and errors;
- candidate soundness/completeness harness;
- opaque-ID and knowledge lifecycle;
