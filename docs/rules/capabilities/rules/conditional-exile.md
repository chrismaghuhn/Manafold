# Capability Specification — rules/conditional-exile

**Capability key/version:** `rules/conditional-exile@0.1.0`
**Lifecycle:** `specified`
**Information risk:** `critical`
**M3 implementation owner:** rules-maintainer
**Mapping boundary:** Selected B2 family: `cap.conditional_exile`

## Purpose

This durable V1 capability covers an exile instruction that applies only when its stated condition or eligibility test is satisfied.

## Supported scope

- Includes: an exile instruction that applies only when its stated condition or eligibility test is satisfied
...
- unconditional exile and a condition that only modifies an already exiled card

## Authority

- `b1-final:sources/m2_5/closures/B1/official_authority_citations.v3.json#cap.conditional_exile`
- `b2-boundary:sources/m2_5/closures/B2/requirement_family_catalog.v1.json#cap.conditional_exile`
