# Capability Specification — rules/triggered-ability

**Capability key/version:** `rules/triggered-ability@0.1.0`
**Lifecycle:** `specified`
**Information risk:** `high`
**M3 implementation owner:** rules-maintainer
**Mapping boundary:** Selected B2 family: `cap.trigger`

## Purpose

This durable V1 capability covers an actual triggered ability with an event condition introduced by when, whenever, or at.

## Supported scope

- Includes: an actual triggered ability with an event condition introduced by when, whenever, or at
...
- reminder text, quoted ability text, or a bare occurrence of a trigger marker without an actual triggered ability

## Authority

- `b1-final:sources/m2_5/closures/B1/official_authority_citations.v3.json#cap.trigger`
- `b2-boundary:sources/m2_5/closures/B2/requirement_family_catalog.v1.json#cap.trigger`
...
- Selected B2 family: `cap.trigger`
- Selected outlier surfaces: Havengul Lich [dynamic-ability-grant;delayed-trigger;graveyard-permission]; Trostani Discordant [control-change;owner-controller;tokens;continuous-pt].
