# Capability Specification — rules/ownership-control

**Capability key/version:** `rules/ownership-control@0.1.0`
**Lifecycle:** `specified`
**Information risk:** `critical`
**M3 implementation owner:** information-safety-maintainer, rules-interaction-maintainer
**Mapping boundary:** Selected B2 family: `cap.owner_controller_separation`

## Purpose

This durable V1 capability covers an effect whose result distinguishes an object's owner from its current controller.

## Supported scope

- Includes: an effect whose result distinguishes an object's owner from its current controller
...
- an effect using only controller or only owner without the separation

## Authority

- `b1-final:sources/m2_5/closures/B1/official_authority_citations.v3.json#cap.owner_controller_separation`
- `b2-boundary:sources/m2_5/closures/B2/requirement_family_catalog.v1.json#cap.owner_controller_separation`
- `commander-policy:CG-COMMAND-ZONE`
- `comprehensive-rules:CR-109-objects`
...
- Selected B2 family: `cap.owner_controller_separation`
- Selected outlier surfaces: Gravespawn Sovereign [control-change;tap-subset-cost;reanimation]; Trostani Discordant [control-change;owner-controller;tokens;continuous-pt].
