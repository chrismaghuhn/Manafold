# Capability Specification — mechanic/token-exception

**Capability key/version:** `mechanic/token-exception@0.1.0`
**Lifecycle:** `specified`
**Information risk:** `high`
**M3 implementation owner:** rules-interaction-maintainer
**Mapping boundary:** Selected B2 family: `cap.token_exception`

## Purpose

This durable V1 capability covers an effect that distinguishes nontoken objects from tokens in its eligibility or result.

## Supported scope

- Includes: an effect that distinguishes nontoken objects from tokens in its eligibility or result
...
- ordinary token creation or a token mention without a nontoken-versus-token rule distinction

## Authority

- `b1-final:sources/m2_5/closures/B1/official_authority_citations.v3.json#cap.token_exception`
- `b2-boundary:sources/m2_5/closures/B2/requirement_family_catalog.v1.json#cap.token_exception`
