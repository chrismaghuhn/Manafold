# Capability Specification — mechanic/scaled-token-creation

**Capability key/version:** `mechanic/scaled-token-creation@0.1.0`
**Lifecycle:** `specified`
**Information risk:** `high`
**M3 implementation owner:** rules-interaction-maintainer
**Mapping boundary:** Selected B2 family: `cap.scaled_token_creation`

## Purpose

This durable V1 capability covers token creation whose quantity is determined by X, a count, equal-to relation, or for-each scaling.

## Supported scope

- Includes: token creation whose quantity is determined by X, a count, equal-to relation, or for-each scaling
...
- a fixed single token, a fixed plurality without scaling, or a power/toughness number that is not token quantity

## Authority

- `b1-final:sources/m2_5/closures/B1/official_authority_citations.v3.json#cap.scaled_token_creation`
- `b2-boundary:sources/m2_5/closures/B2/requirement_family_catalog.v1.json#cap.scaled_token_creation`
...
- Selected B2 family: `cap.scaled_token_creation`
