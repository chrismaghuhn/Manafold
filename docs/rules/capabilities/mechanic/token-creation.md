# Capability Specification — mechanic/token-creation

**Capability key/version:** `mechanic/token-creation@0.1.0`
**Lifecycle:** `specified`
**Information risk:** `high`
**M3 implementation owner:** rules-interaction-maintainer
**Mapping boundary:** Selected B2 family: `cap.token_creation`

## Purpose

This durable V1 capability covers an instruction that creates one or more token objects.

## Supported scope

- Includes: an instruction that creates one or more token objects
...
- a token reference, token descriptor, or token copy without an actual creation instruction

## Authority

- `b1-final:sources/m2_5/closures/B1/official_authority_citations.v3.json#cap.token_creation`
- `b2-boundary:sources/m2_5/closures/B2/requirement_family_catalog.v1.json#cap.token_creation`
...
- Selected B2 family: `cap.token_creation`
- Selected outlier surfaces: Ajani, Caller of the Pride [planeswalker;loyalty;continuous-ability-grant;tokens]; Necrotic Hex [multi-actor-resolution;continuation;simultaneous-sacrifice;tokens]; Rootborn Defenses [populate;bounded-token-copy;indestructible]; Trostani Discordant [control-change;owner-controller;tokens;continuous-pt].
