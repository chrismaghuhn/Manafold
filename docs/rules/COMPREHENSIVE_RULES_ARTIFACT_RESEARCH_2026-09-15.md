...
The official page itself is the source for the variant inventory; the exact byte lengths and SHA-256 values above are measurements of the direct downloads at the stated timestamps.
...
Rationale: TXT is the smallest and most direct machine-readable representation for deterministic clause extraction and citation. It avoids PDF layout interpretation and DOCX ZIP/XML packaging while remaining an official Wizards artifact. Keep the PDF and DOCX identities above as official presentation/audit variants, not as separate semantic rule versions. If a case needs visual pagination evidence, use the PDF as a secondary cross-reference while retaining the TXT binding as the semantic source.
