# ADR 0041: Capability-Oriented Semantic Domains and Explicit Semantic Ownership
...
This ADR deliberately does **not** freeze:
...
notes were incorporated before acceptance. The post-`M2.Final` drift review
found no material contradiction between the candidate and the consolidated M2
architecture (rules/environment transaction split, `mtgml-state` sole state
ownership, Decision V2 identity, capability Registry V1 fields, and the Issue
