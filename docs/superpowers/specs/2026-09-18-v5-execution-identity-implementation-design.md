...
- **Generation contract (Fix-03 — closes "who generates the constants"):** the ID constants are produced by a checked-in GENERATOR (proposed `scripts/generate_semantic_contract_catalog.py`), which derives them deterministically from the checked-in manifest constants via the canonical construction (Python mirror of the §9 functions in `python/src/mtgml/persistence.py`) and emits them as Rust source in the catalog module, marked with a generated-file banner. Layered discipline: the manifest constants are the hand-authored SOURCE of semantic meaning; the generator derives the ID constants deterministically; the generated constants are checked in; and the §19.4 recompute-KAT independently recomputes and verifies them (Rust at every test run; Python mirror in the Python suite). A KAT alone would make a manually typed digest verifiable but not GENERATED — the generator closes that gap, and the two independent recomputations (generator at regeneration time, KAT at every build) plus the checked-in values must all agree, so neither the generator nor a manual edit can silently drift.
- **Construction (ADR-mandated single authoritative path):** a single deterministic constructor exposes the checked-in manifest constants and their ID constants (it computes no digests; it hands out the frozen values); only the §19.4 recompute-KAT ever re-derives the IDs from the manifests, and the S1 slice updates manifest constants, ID constants, generator output, and KAT in the same change. Production values for the legacy synthetic contract:
...
- the new `scripts/run_v5_execution_identity_gate.py` is added to `run_checks.py`'s `FAST` command list (verified at baseline: PR Fast/PR Integration/Windows Setup Smoke run `run_checks.py fast`; PR Integration and Nightly run `integration`/`certification`, both of which append their extras ON TOP of FAST — so FAST placement covers all hosted profiles automatically); locally, `justfile contracts` additionally invokes the gate directly beside `verify_repository.py` so the maintainer loop covers it without the full FAST profile;
...
1. `SyntheticM1RulesKernel` (`crates/mtgml-rules/src/synthetic.rs:54`, `#[derive(Debug, Default)]`) — the only `Default` derive on a semantics-carrying type in rules/environment/replay (grep-verified). **Decision (Fix-03, closing ADR 0055's `PROGRAM_OWNS_ALL_KERNEL_ENTRYPOINTS = YES` named-constructor-only contract): REMOVE the `Default` derive AND eliminate every direct unit-struct literal as a semantic construction path.** The literal `SyntheticM1RulesKernel` is currently constructed directly at `synthetic.rs:113`, `synthetic.rs:140`, and `synthetic.rs:165` (production) plus `tests/checkpoint_replay.rs:741` and `tests/forced_progress.rs:198` (test) — complete census, grep-verified at baseline; a unit-struct literal is syntactically explicit but is NOT a named constructor and does not satisfy the ADR. After V5 the ONLY kernel construction boundary is a named program-owned factory — proposed shape `RulesKernelFactory::for_program(program_kind) -> Result<Box<dyn RulesKernel>, UnsupportedProgram>` — returning the existing `SyntheticM1RulesKernel` (behavior unchanged) for `SyntheticRulesCompat` and `Err(UnsupportedProgram)` for `MagicRules` (no production Magic contract exists pre-S1, §25). The unit struct becomes module-private (or loses public literal construction); every listed site migrates explicitly at compile time. No naked `SyntheticM1RulesKernel` literal remains as a semantic construction path.
