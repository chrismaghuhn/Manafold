...
- `scripts/run_m2_b_contract_cut.py`: SPLIT per ADR §2.15 — keep historical M2 assertions byte-identical (V4-as-of-M2 evidence + V3-non-resurgence); move the "current successor identity" block (currently asserting V4 tokens) into a new V5 gate script. `M2_B_SCRIPT_RENAME_POLICY = NO_RENAME`: the split is LOGICAL ONLY — `scripts/run_m2_b_contract_cut.py` retains its existing path/name (ADR §2.15 mandates a split of concerns, not a rename; no ADR-numbered filename exists or is created), and the retained historical script gains ONLY the explicitly permitted historical-posture line (not a re-check): historical M2 evidence was captured when V4 WAS current; those tokens are frozen as-of-M2, never re-validated against a later master; the script is never re-pointed at a newer engine version. History and currentness are never mixed in one runner again.
...
1. `SyntheticM1RulesKernel` (`crates/mtgml-rules/src/synthetic.rs:54`, `#[derive(Debug, Default)]`) — the only `Default` derive on a semantics-carrying type in rules/environment/replay (grep-verified). **Decision (Fix-03 + Fix-04 + Fix-07, closing ADR 0055's `PROGRAM_OWNS_ALL_KERNEL_ENTRYPOINTS = YES` completely): REMOVE the `Default` derive AND replace every direct kernel construction with ONE named program-owned kernel boundary carrying BOTH mandatory kernel entry points.** Verified baseline reality: the `RulesKernel` trait (`transition.rs:18`) has ONLY `apply`; forced progress lives on the inherent method `advance_forced_progress` (`synthetic.rs:122`), called by the production forced-progress path at `synthetic/commit.rs:97` and `:219` — so a `Box<dyn RulesKernel>` boundary would silently LOSE the second mandatory entry point. Therefore the program-owned boundary is an OPAQUE PUBLIC ADAPTER STRUCT wrapping a PRIVATE inner enum — NOT a bare trait object, and NOT a public enum exposing the private kernel payload type (a public variant field of type `SyntheticM1RulesKernel` would make the "kernel becomes module-private" visibility requirement uncompilable for external consumers):

```rust
pub struct ProgramKernelV1 {       // owner: mtgml-rules (new module, e.g. program_kernel.rs)
    inner: ProgramKernelInner,     // public, opaque struct; PRIVATE inner payload
}                                  // (Fix-07: spec/plan-synced representation — no public
                                   //  variant exposing a private payload type)
...
`for_program(SyntheticRulesCompat)` constructs `ProgramKernelInner::SyntheticLegacy(SyntheticM1RulesKernel)` (behavior unchanged); `for_program(MagicRules)` returns `Err(ProgramKernelConstructionErrorV1::UnsupportedProgram)` (no production Magic contract pre-S1; error type per §18/§5). Visibility/export contract (Fix-07, spec/plan-synced): `ProgramKernelV1` and `ProgramKernelConstructionErrorV1` are PUBLIC exports of mtgml-rules (owned by the new kernel-boundary module, e.g. `crates/mtgml-rules/src/program_kernel.rs`, re-exported from the crate root); `ProgramKernelInner` is PRIVATE; `SyntheticM1RulesKernel` is a PRIVATE implementation detail of mtgml-rules and the existing public re-export at `crates/mtgml-rules/src/lib.rs:24` (`pub use synthetic::{validate_synthetic_runtime_state, SyntheticM1RulesKernel};`) is reduced to `pub use synthetic::validate_synthetic_runtime_state;` — the kernel name leaves the public surface with the cut. `Default` is removed.

Re-verified literal census (Fix-07 — supersedes the earlier grep-only census; grep re-verified at baseline against live sources): public re-export `crates/mtgml-rules/src/lib.rs:24` (removed, above); production environment sites `crates/mtgml-environment/src/synthetic.rs:11` (external import), `:113` (`new()`), `:140` (`from_checkpoint()`), `:165` (`restore()` reset), struct field `:81` (becomes `ProgramKernelV1`); rules-internal tests `crates/mtgml-rules/src/tests.rs:135`, `:154`, `:170`; `crates/mtgml-rules/src/tests/determinism.rs:8`, `:9`, `:34`, `:35`; `crates/mtgml-rules/src/tests/forced_progress.rs:14`; `crates/mtgml-rules/src/tests/synthetic_program.rs:337`, `:425`; `crates/mtgml-rules/src/tests/transition_contract.rs:84`, `:100`, `:151`, `:165`, `:178`, `:216`; external historical test `crates/mtgml-rules/tests/p0_red.rs:7` (import), `:55` (construction); environment tests `crates/mtgml-environment/src/tests/checkpoint_replay.rs:741`, `crates/mtgml-environment/src/tests/forced_progress.rs:198` (construction) and `:199` (forced-progress call — all forced-progress call sites `commit.rs:97`/`:219` and `tests/forced_progress.rs:199` switch from the inherent method to the `ProgramKernelV1` dispatch). Allowed to REMAIN (internal implementation references, never constructions): the declaration and impls in `crates/mtgml-rules/src/synthetic.rs` (`:55`, `:57`, `:102`), the child-module impl `crates/mtgml-rules/src/synthetic/stages.rs` (`:13` import, `:18` impl), and the single `ProgramKernelInner::SyntheticLegacy(SyntheticM1RulesKernel)` construction owned by the kernel-boundary module. Every other site migrates explicitly at compile time; no naked `SyntheticM1RulesKernel` literal remains as a semantic construction path, no public kernel-name surface remains, and both mandatory entry points (trusted response execution + forced progress execution) are program-owned.
