...
| object_became_target | Per current GameObjectId set/marker | Emberheart’s first own spell/ability target condition this turn; new zone incarnation is a new object. |
...
2. TurnHistoryState: closed named fields in section 5; keyed only by PlayerId, current GameObjectId and—only after ability authority resolves—AbilityKey; zero/false entries omitted; checked values; explicit current-turn reset; public event evidence and hidden-ID-safe projection.
...
5. FaceState: current FaceKey on object; validate membership in content definition; change on transform without new incarnation; reset/choose face per zone-entry rules; event order for Ojer’s return-then-transform.
...
- freeze the exact TurnHistory fields/reset/update rules, including the AbilityKey-dependent row;
...
- specify FaceKey storage/default/transform/zone-change and Ojer return event sequence;
