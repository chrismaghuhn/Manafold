...
`cargo-audit` `0.21.2` and PyPA `pip-audit` `2.10.1`. Install them in an
...
2.10.1 is installed; `cargo-audit` is discovered as `cargo-audit` on `PATH` or
can be supplied with `--rust-audit-executable`. The script audits
