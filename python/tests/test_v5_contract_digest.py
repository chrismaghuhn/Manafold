...
class SemanticContractDigestTests(unittest.TestCase):
    ...
    def test_schema_domain_disagreement_is_detectable_and_bound_to_identity(self) -> None:
        ...
        # decode path. CURRENT_V5_PLAN_OWNER = NONE — the merged V5 plan
        # contains no task that introduces such a decoder (Task 8 semantic
        # admission works on already-constructed typed manifests, not on
        # canonical-CBOR contract envelopes). The obligation is tracked
        # durably in the Task-2 section of
