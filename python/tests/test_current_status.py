...
class CurrentStatusEntryPointTests(unittest.TestCase):
    ...
    def test_plan_acceptance_is_separate_from_execution_authorization(self) -> None:
        ...
        self.assertIn("- **Status:** accepted (PR #184 merged)", adr)
        self.assertIn("**Status:** accepted (PR #184 merged)", foundation)
        ...
        self.assertIn("M3_PRE_T0_HARDENING = COMPLETE / ACCEPTED", adr)
        self.assertIn("M3_PLAN_STATUS = ACCEPTED_PENDING_REAUTHORIZATION", adr)
        self.assertIn("NEXT_GATE = EXACT_MASTER_REAUTHORIZATION", adr)
        ...
        self.assertNotIn("M3_PRE_T0_HARDENING = CANDIDATE", adr)
        self.assertIn("M3_PRE_T0_HARDENING = COMPLETE / ACCEPTED", foundation)
        self.assertIn("M3_PLAN_STATUS = ACCEPTED_PENDING_REAUTHORIZATION", foundation)
        self.assertIn("NEXT_GATE = EXACT_MASTER_REAUTHORIZATION", foundation)
