...
from .canonical import parse_u64_number, parse_uint, require_digest, require_exact_keys, require_nonempty, uint_wire
...
class KernelIdentityV1:
    ...
    def from_wire(cls, value: object) -> "KernelIdentityV1":
        ...
        return cls(*(require_nonempty(obj[key], key) for key in ("implementation_id", "semantic_version", "build_profile")))

    def to_wire(self) -> dict[str, object]:
        return {"build_profile": require_nonempty(self.build_profile, "build_profile"), "implementation_id": require_nonempty(self.implementation_id, "implementation_id"), "semantic_version": require_nonempty(self.semantic_version, "semantic_version")}
...
class ReplaySchemaVersionsV1:
    ...
    def from_wire(cls, value: object) -> "ReplaySchemaVersionsV1":
        keys = {"observation", "information_state", "decision", "decision_response", "observed_event", "player_step", "replay_step"}
        obj = require_exact_keys(value, keys)
        return cls(**{key: require_nonempty(obj[key], key) for key in keys})

    def to_wire(self) -> dict[str, object]:
        return {key: require_nonempty(getattr(self, key), key) for key in ("decision", "decision_response", "information_state", "observation", "observed_event", "player_step", "replay_step")}
...
class RandomnessIdentityV1:
    ...
    def from_wire(cls, value: object) -> "RandomnessIdentityV1":
        ...
        if not isinstance(seed, str) or len(seed) != 64 or any(ch not in "0123456789abcdef" for ch in seed):
            raise WireError("semantic.replay_manifest", "root seed is not canonical hex")
        return cls(require_nonempty(obj["algorithm_id"], "algorithm_id"), require_nonempty(obj["derivation_version"], "derivation_version"), seed)

    def to_wire(self) -> dict[str, object]:
        return RandomnessIdentityV1.from_wire({"algorithm_id": self.algorithm_id, "derivation_version": self.derivation_version, "root_seed_hex": self.root_seed_hex})._raw()

    def _raw(self) -> dict[str, object]:
        return {"algorithm_id": self.algorithm_id, "derivation_version": self.derivation_version, "root_seed_hex": self.root_seed_hex}
...
class DeckIdentityV1:
    ...
    def from_wire(cls, value: object) -> "DeckIdentityV1":
        ...
        return cls(parse_uint(obj["player"]), require_nonempty(obj["deck_id"], "deck_id"), require_digest(obj["digest"]))

    def to_wire(self) -> dict[str, object]:
        return {"deck_id": require_nonempty(self.deck_id, "deck_id"), "digest": require_digest(self.digest), "player": uint_wire(self.player)}
...
class ReplayManifestV1:
    ...
    def from_wire(cls, value: object) -> "ReplayManifestV1":
        obj = require_exact_keys(value, {"schema_version", "engine_build", "kernel", "rules_snapshot", "format_policy_snapshot", "oracle_snapshot", "card_bundle", "schemas", "randomness", "decks", "initial_state_revision", "initial_state_digest"})
...
class ReplayStepV1:
    ...
    def from_wire(cls, value: object) -> "ReplayStepV1":
        obj = require_exact_keys(value, {"step_index", "state_revision_before", "response", "accepted", "state_revision_after", "state_digest_after"})
        if not isinstance(obj["accepted"], bool):
            raise WireError("decode.invalid_json", "accepted must be boolean")
        return cls(parse_u64_number(obj["step_index"]), parse_uint(obj["state_revision_before"]), DecisionResponse.from_wire(obj["response"]), obj["accepted"], parse_uint(obj["state_revision_after"]), require_digest(obj["state_digest_after"]))

    def to_wire(self) -> dict[str, object]:
        return {"accepted": self.accepted, "response": self.response.to_wire(), "state_digest_after": require_digest(self.state_digest_after), "state_revision_after": uint_wire(self.state_revision_after), "state_revision_before": uint_wire(self.state_revision_before), "step_index": parse_u64_number(self.step_index)}
...
class AuthoritativeReplayV1:
    ...
    def from_wire(cls, value: object) -> "AuthoritativeReplayV1":
        obj = require_exact_keys(value, {"schema_version", "manifest", "steps", "final_state_revision", "final_state_digest"})
        ...
        result = cls(REPLAY_FILE_SCHEMA, ReplayManifestV1.from_wire(obj["manifest"]), tuple(ReplayStepV1.from_wire(item) for item in obj["steps"]), parse_uint(obj["final_state_revision"]), require_digest(obj["final_state_digest"]))
        result.validate()
        return result
    ...
    def validate(self) -> None:
        ...
        for index, step in enumerate(self.steps):
            if step.step_index != index or step.state_revision_before != revision or step.response.state_revision != revision:
        ...
        elif self.final_state_revision != self.manifest.initial_state_revision or self.final_state_digest != self.manifest.initial_state_digest:
            raise WireError("semantic.replay", "empty replay does not preserve initial identity")

    def to_wire(self) -> dict[str, object]:
        self.validate()
        return {"final_state_digest": require_digest(self.final_state_digest), "final_state_revision": uint_wire(self.final_state_revision), "manifest": self.manifest.to_wire(), "schema_version": self.schema_version, "steps": [step.to_wire() for step in self.steps]}
