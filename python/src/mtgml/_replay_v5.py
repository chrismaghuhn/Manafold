...
class ReplaySchemaVersionsV5:
    ...
    @classmethod
    def from_wire(cls, value: object) -> ReplaySchemaVersionsV5:
        keys = {
            "observation",
...
@dataclass(frozen=True, slots=True)
class InitialEnvironmentIdentityV5:
    state_revision: int
    ...
    @classmethod
    def from_wire(cls, value: object) -> InitialEnvironmentIdentityV5:
        obj = require_exact_keys(
            value,
            {
                "state_revision",
    ...
    def recompute_checkpoint_digest(self) -> str:
        ...
        return calculate_checkpoint_digest_v5(
    ...
    def validate(self, *, error_code: str = "semantic.replay") -> None:
        if (
            self.checkpoint_codec_identity.codec_id != CHECKPOINT_CODEC_ID_V5
            or self.checkpoint_codec_identity.semantic_version != CHECKPOINT_CODEC_VERSION_V5
        ):
            raise WireError(error_code, "checkpoint codec identity is not V5")
...
class ReplayManifestV5:
    ...
    @classmethod
    def from_wire(cls, value: object) -> ReplayManifestV5:
        obj = require_exact_keys(
            value,
            {
                "schema_version",
        ...
        if not isinstance(obj["schema_version"], str) or not isinstance(obj["decks"], list):
            raise WireError("decode.invalid_json", "unsupported replay manifest V5")
        result = cls(
            ...
            ReplaySchemaVersionsV5.from_wire(obj["schemas"]),
            RandomnessIdentityV2.from_wire(obj["randomness"]),
            tuple(DeckIdentityV1.from_wire(item) for item in obj["decks"]),
            InitialEnvironmentIdentityV5.from_wire(obj["initial_identity"]),
            ExecutionIdentityV1.from_wire(obj["execution_identity"]),
    ...
    def validate(self) -> None:
        if self.schema_version != REPLAY_MANIFEST_SCHEMA_V5:
            raise WireError("semantic.replay_manifest", "unsupported replay manifest V5")
        for label, field in (
        ...
        if self.schemas.replay_step != REPLAY_STEP_SCHEMA_V5:
            raise WireError("semantic.replay_manifest", "replay-step schema is not V5")
        if not self.decks:
    ...
    def to_wire(self) -> dict[str, object]:
        ...
        return {
            ...
            "schema_version": REPLAY_MANIFEST_SCHEMA_V5,
...
class ReplayStepV5:
    ...
    @classmethod
    def from_wire(cls, value: object) -> ReplayStepV5:
        obj = require_exact_keys(
            value,
            {
                "step_index",
...
@dataclass(frozen=True, slots=True)
class AuthoritativeReplayV5:
    schema_version: str
    manifest: ReplayManifestV5
    steps: tuple[ReplayStepV5, ...]
    final_identity: InitialEnvironmentIdentityV5

    @classmethod
    def from_wire(cls, value: object) -> AuthoritativeReplayV5:
        ...
        if not isinstance(obj["schema_version"], str) or not isinstance(obj["steps"], list):
            raise WireError("decode.invalid_json", "unsupported authoritative replay V5")
        try:
            manifest = ReplayManifestV5.from_wire(obj["manifest"])
        except WireError as exc:
            raise WireError("semantic.replay", exc.message) from exc
        result = cls(
            str(obj["schema_version"]),
            manifest,
            tuple(ReplayStepV5.from_wire(item) for item in obj["steps"]),
            InitialEnvironmentIdentityV5.from_wire(obj["final_identity"]),
        )
        result.validate()
        return result
    ...
    def validate(self) -> None:
        if self.schema_version != REPLAY_FILE_SCHEMA_V5:
            raise WireError("semantic.replay", "unsupported authoritative replay V5")
        ...
        for index, step in enumerate(self.steps):
            ...
            previous = InitialEnvironmentIdentityV5(
    ...
    def to_wire(self) -> dict[str, object]:
        ...
        return {
            ...
            "schema_version": REPLAY_FILE_SCHEMA_V5,
