use mtgml_model::{
    ...
    ExecutionIdentityV1, ExecutionProgramV1, FullStateDigestV4, PlayerId, RulesAuthorityV1,
...
fn v5_identity(
    ...
    let full_state_digest = FullStateDigestV4::from_digest_bytes([digest_byte; 32]);
...
fn print_all_fixtures() {
    ...
    let replay = mtgml_replay::AuthoritativeReplayV5 {
        schema_version: "authoritative-replay.v5".to_string(),
        manifest: manifest2,
        steps: vec![],
    ...
    // Negative: wrong schema version
    {
        ...
        m.schema_version = "replay-manifest.v4".to_string();
        let value = serde_json::to_value(&m).unwrap();
