...
use mtgml_model::{
    CheckpointCodecIdentity, CheckpointDigestV5, ContentDigest, EnvironmentLimitCounters,
    EpisodeStatus, ExecutionIdentityV1, FullStateDigestV4, PlayerId, RulesAuthorityV1,
    RulesContractManifestV1, SemanticContractIdV1, SemanticContractManifestV1, StateRevision,
};
...
pub const REPLAY_MANIFEST_SCHEMA_V5: &str = "replay-manifest.v5";
pub const REPLAY_FILE_SCHEMA_V5: &str = "authoritative-replay.v5";
pub const REPLAY_STEP_SCHEMA_V5: &str = "replay-step.v5";
...
pub struct ReplaySchemaVersionsV5 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct InitialEnvironmentIdentityV5 {
...
impl InitialEnvironmentIdentityV5 {
    pub fn validate(&self) -> Result<(), ReplayValidationError> {
        self.episode_status
            .validate()
            .map_err(|_| ReplayValidationError::CheckpointIdentity)?;
        if self.checkpoint_codec_identity.codec_id != CHECKPOINT_CODEC_ID_V5
            || self.checkpoint_codec_identity.semantic_version != CHECKPOINT_CODEC_VERSION_V5
        {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SemanticContractMaterialV5 {
...
impl SemanticContractMaterialV5 {
    pub fn validate(&self) -> Result<(), ReplayValidationError> {
        let recomputed =
        ...
        if recomputed != self.semantic_contract_id {
            return Err(ReplayValidationError::SemanticContractMismatch);
        }
        let recomputed_rules =
            mtgml_persistence::semantic_contract_digest::calculate_rules_contract_id_v1(
                &self.rules_manifest,
            )
            .map_err(|_| ReplayValidationError::SemanticContractMismatch)?;
        ...
        if self.manifest.format_contract_id.is_some() || self.manifest.content_contract_id.is_some()
        {
            return Err(ReplayValidationError::SemanticContractMismatch);
        }
        Ok(())
...
impl ReplayManifestV5 {
    pub fn validate(&self) -> Result<(), ReplayValidationError> {
        if self.schema_version != REPLAY_MANIFEST_SCHEMA_V5 {
        ...
        if self.randomness.contract_id != "mtgml.rng.v1"
            ...
            || self.schemas.observation_payload_codec != "synthetic-m3-observation.v1"
            ...
            || self.schemas.replay_step != REPLAY_STEP_SCHEMA_V5
        {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ReplayStepV5 {
    pub step_index: u64,
    pub actor: PlayerId,
    pub checkpoint_digest_before: CheckpointDigestV5,
    pub state_revision_before: StateRevision,
    pub response: DecisionResponseV2,
    pub accepted: bool,
    pub state_revision_after: StateRevision,
    pub full_state_digest_after: FullStateDigestV4,
    pub episode_status_after: EpisodeStatus,
    pub environment_limit_counters_after: EnvironmentLimitCounters,
    pub checkpoint_digest_after: CheckpointDigestV5,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AuthoritativeReplayV5 {
    pub schema_version: String,
    pub manifest: ReplayManifestV5,
    pub steps: Vec<ReplayStepV5>,
    pub final_identity: InitialEnvironmentIdentityV5,
}

impl AuthoritativeReplayV5 {
    pub fn validate(&self) -> Result<(), ReplayValidationError> {
        if self.schema_version != REPLAY_FILE_SCHEMA_V5 {
            return Err(ReplayValidationError::SchemaVersion);
        }
        ...
        for (index, step) in self.steps.iter().enumerate() {
            ...
            let next = InitialEnvironmentIdentityV5 {
...
pub struct ReplayRecorderV5 {
    manifest: ReplayManifestV5,
    steps: Vec<ReplayStepV5>,
    final_identity: InitialEnvironmentIdentityV5,
}

impl ReplayRecorderV5 {
    pub fn new(manifest: ReplayManifestV5) -> Result<Self, ReplayValidationError> {
        manifest.validate()?;
        Ok(Self {
    ...
    pub fn append(&mut self, step: ReplayStepV5) -> Result<(), ReplayValidationError> {
        let mut steps = self.steps.clone();
        steps.push(step);
        let candidate = AuthoritativeReplayV5 {
        ...
        let final_identity = InitialEnvironmentIdentityV5 {
            state_revision: step.state_revision_after,
            full_state_digest: step.full_state_digest_after.clone(),
            episode_status: step.episode_status_after.clone(),
            environment_limit_counters: step.environment_limit_counters_after.clone(),
    ...
    pub fn export(&self) -> Result<AuthoritativeReplayV5, ReplayValidationError> {
    ...
    pub fn manifest(&self) -> &ReplayManifestV5 {
        &self.manifest
    }
...
fn calculate_checkpoint_digest(
    identity: &InitialEnvironmentIdentityV5,
) -> Result<CheckpointDigestV5, ReplayValidationError> {
