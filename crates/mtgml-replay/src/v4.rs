...
use mtgml_model::{
    CheckpointCodecIdentity, CheckpointDigestV4, ContentDigest, EnvironmentLimitCounters,
    EpisodeStatus, FullStateDigestV4, PlayerId, StateRevision,
};
...
pub const REPLAY_MANIFEST_SCHEMA_V4: &str = "replay-manifest.v4";
pub const REPLAY_FILE_SCHEMA_V4: &str = "authoritative-replay.v4";
pub const REPLAY_STEP_SCHEMA_V4: &str = "replay-step.v4";

const CHECKPOINT_CODEC_ID_V4: &str = "in-memory-reference";
const CHECKPOINT_CODEC_VERSION_V4: &str = "4";
...
pub struct InitialEnvironmentIdentityV4 {
...
impl InitialEnvironmentIdentityV4 {
    pub fn validate(&self) -> Result<(), ReplayValidationError> {
        self.episode_status
            .validate()
            .map_err(|_| ReplayValidationError::CheckpointIdentity)?;
        if self.checkpoint_codec_identity.codec_id != CHECKPOINT_CODEC_ID_V4
            || self.checkpoint_codec_identity.semantic_version != CHECKPOINT_CODEC_VERSION_V4
        {
...
pub struct ReplayManifestV4 {
    ...
    pub initial_identity: InitialEnvironmentIdentityV4,
}
...
impl ReplayManifestV4 {
    pub fn validate(&self) -> Result<(), ReplayValidationError> {
        ...
        if self.randomness.contract_id != "mtgml.rng.v1"
            ...
            || self.schemas.replay_step != REPLAY_STEP_SCHEMA_V4
        {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ReplayStepV4 {
    pub step_index: u64,
    pub actor: PlayerId,
    pub checkpoint_digest_before: CheckpointDigestV4,
    ...
    pub checkpoint_digest_after: CheckpointDigestV4,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AuthoritativeReplayV4 {
    pub schema_version: String,
    pub manifest: ReplayManifestV4,
    pub steps: Vec<ReplayStepV4>,
    pub final_identity: InitialEnvironmentIdentityV4,
}

impl AuthoritativeReplayV4 {
    pub fn validate(&self) -> Result<(), ReplayValidationError> {
        ...
        for (index, step) in self.steps.iter().enumerate() {
            ...
            let next = InitialEnvironmentIdentityV4 {
...
pub struct ReplayRecorderV4 {
    manifest: ReplayManifestV4,
    steps: Vec<ReplayStepV4>,
    final_identity: InitialEnvironmentIdentityV4,
}

impl ReplayRecorderV4 {
    pub fn new(manifest: ReplayManifestV4) -> Result<Self, ReplayValidationError> {
        manifest.validate()?;
        Ok(Self {
    ...
    pub fn append(&mut self, step: ReplayStepV4) -> Result<(), ReplayValidationError> {
        let mut steps = self.steps.clone();
        steps.push(step);
        let candidate = AuthoritativeReplayV4 {
        ...
        let final_identity = InitialEnvironmentIdentityV4 {
            state_revision: step.state_revision_after,
            full_state_digest: step.full_state_digest_after.clone(),
            episode_status: step.episode_status_after.clone(),
            environment_limit_counters: step.environment_limit_counters_after.clone(),
    ...
    pub fn export(&self) -> Result<AuthoritativeReplayV4, ReplayValidationError> {
    ...
    pub fn manifest(&self) -> &ReplayManifestV4 {
        &self.manifest
    }
...
fn calculate_checkpoint_digest(
    identity: &InitialEnvironmentIdentityV4,
) -> Result<CheckpointDigestV4, ReplayValidationError> {
