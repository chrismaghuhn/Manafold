...
mod tests {
    ...
    use mtgml_model::{
        DecisionId, FullStateDigestV5, PlayerDecisionIdV1, PlayerOutcome, PlayerResult,
    ...
    fn exact_transition_state_digest_failure_is_detailed() {
        let (before, result, mut expected) = lifecycle_conformance_case();
        expected.expected_state_digest =
            FullStateDigestV5::parse("ff".repeat(32)).expect("synthetic digest");
    ...
    fn exact_product_digest_failure_is_detailed() {
        ...
        let wrong_digest = FullStateDigestV5::parse("ff".repeat(32)).expect("synthetic digest");
