//! The single typed producer for the current full-state V4 digest.
//!
//! The conversion deliberately does not use Rust/Serde output. Every field is
//! mapped to the fixed semantic CBOR layout from STATE_HASHING.md. V4 adds
//! only the closed Initial-Foundation source facts to the existing semantic
//! components.
...
use mtgml_model::FullStateDigestV4;
...
pub const FULL_STATE_DIGEST_INPUT_SCHEMA_V4: &str = "full-state-digest-input.v4";

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct FullStateDigestInputV4 {
    pub revision: u64,
    pub core: Value,
    pub zones: Value,
...
impl FullStateDigestInputV4 {
    pub fn canonical_value(&self) -> Result<Value, StateDigestError> {
        ...
        Ok(Value::Array(vec![
            Value::Text(FULL_STATE_DIGEST_INPUT_SCHEMA_V4.to_owned()),
            Value::Text(FULL_STATE_DIGEST_DOMAIN_V4.to_owned()),
...
pub(crate) fn calculate_full_state_digest_v4(
    input: &FullStateDigestInputV4,
) -> Result<FullStateDigestV4, StateDigestError> {
    let payload = input.canonical_payload()?;
    let envelope = envelope::encode_envelope(
        FULL_STATE_DIGEST_DOMAIN_V4,
        FULL_STATE_DIGEST_INPUT_SCHEMA_V4,
        &payload,
    )
    .map_err(StateDigestError::Persistence)?;
    Ok(FullStateDigestV4::from_digest_bytes(
        envelope::hash_envelope(&envelope),
    ))
}

pub(crate) fn calculate_full_state_digest_v4_for_state(
    state: &EngineState,
) -> Result<FullStateDigestV4, StateDigestError> {
    calculate_full_state_digest_v4(&full_state_digest_input(state)?)
}

pub(crate) fn full_state_digest_input(
    state: &EngineState,
) -> Result<FullStateDigestInputV4, StateDigestError> {
    crate::validation::validate_engine_state(state)
        .map_err(|_| StateDigestError::StateInvariant)?;
    Ok(FullStateDigestInputV4 {
