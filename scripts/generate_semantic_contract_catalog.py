...
def render_rules_execution_generated(
    ...
    """Render the minimal Magic execution-support view for mtgml-rules.

    The view is derived from the same catalog source as the manifest catalog,
    but emits only identities, the opaque execution profile, capability
    authorization, and the program/contract support query needed at runtime.
    """
    ...
    for entry in catalog["entries"]:
        ...
        if entry_id == RULES_ENTRY_ID:
            magic_entry = entry
        elif entry_id == "synthetic_legacy_default":
    ...
    _, semantic_id = derive_ids(magic_entry)
    _, synthetic_semantic_hex = derive_ids(synthetic_entry)

    closure = magic_entry.get("capability_closure")
    if closure is None:
        closure = []
    if not isinstance(closure, list):
        raise SystemExit("capability_closure must be null or a list")
    has_turn_structure = any(
        isinstance(capability, dict)
        and capability.get("key") == "rules/turn-structure"
    ...
    lines.append("//")
    lines.append("// Minimal runtime execution-support view for MagicRules.")
    ...
    lines.append("")
    lines.append("use mtgml_model::{ExecutionProgramV1, SemanticContractIdV1};")
    lines.append("")
    lines.append("pub(crate) const MAGIC_TURN_STRUCTURE_SEMANTIC_CONTRACT_HEX: &str =")
    ...
    lines.extend(
        parse_call_lines(
            ...
            "MAGIC_TURN_STRUCTURE_SEMANTIC_CONTRACT_HEX",
        )
    )
    lines.append("}")
    lines.append("")
    ...
    lines.append("        let admitted = magic_turn_structure_0_1_0_semantic_contract_id();")
    lines.append("        self.turn_structure_0_1_0 && self.admitted_contract == admitted")
    lines.append("    }")
    lines.append("}")
    lines.append("")
    ...
    lines.append("}")
    lines.append("")
    lines.append("pub(crate) fn magic_execution_profile(")
