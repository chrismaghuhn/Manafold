...
def render_rules_execution_generated(catalog: dict[str, object]) -> str:
    ...
    lines.append("")
    lines.append("use mtgml_model::{SemanticContractIdV1, SemanticContractManifestV1, RulesContractIdV1};")
    lines.append("")
    ...
    lines.append("pub struct MagicExecutionProfile {")
    lines.append("    pub admitted_contract: SemanticContractIdV1,")
    lines.append("    pub turn_structure_0_1_0: bool,")
    lines.append("}")
    lines.append("")
...
def check_paths(targets: list[Path], catalog: dict[str, object] | None = None) -> int:
    ...
    current SOURCE_PATH) and reports drift against BOTH targets. The CLI adds
    ...
    rules_drift = [RULES_TARGET_PATH] if not generated_bytes_match(RULES_TARGET_PATH, rules_content) else []
    all_drift = drift + rules_drift
    if all_drift:
        print("generated semantic-contract catalog drift:")
        for rel in all_drift:
            print(f"  - {rel}")
        return 1
    return 0
...
def main() -> int:
    ...
    rules_content = render_rules_execution_generated(catalog)
    if args.check:
        drift = [str(t) for t in [TARGET_PATH, RULES_TARGET_PATH]
                 if not generated_bytes_match(t, content if t == TARGET_PATH else rules_content)]
        if drift:
            print("generated semantic-contract catalog drift:")
            for rel in drift:
                print(f"  - {rel}")
